  ...
  return (
    <div
      className={`rounded-2xl border transition-all duration-200 hover:-translate-y-1 flex flex-col justify-between space-y-5 group h-full ${
        featured
          ? "border-brand-soft/80 dark:border-zinc-700 hover:border-brand-indigo/60 dark:hover:border-zinc-600 bg-gradient-to-br from-white via-white to-brand-light/20 dark:from-zinc-850 dark:via-zinc-850 dark:to-zinc-800 p-6 sm:p-7 shadow-sm hover:shadow-card-hover"
          : "border-slate-200 dark:border-zinc-700 hover:border-slate-300 dark:hover:border-zinc-600 bg-white dark:bg-zinc-850 p-6 shadow-sm hover:shadow-card-hover"
      }`}
    >
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <div className="w-10 h-10 rounded-xl bg-brand-light dark:bg-zinc-800 text-brand-dark dark:text-zinc-200 flex items-center justify-center border border-brand-soft/40 dark:border-zinc-700 group-hover:scale-105 transition-transform">
        ...
        <div>
          <h3
            className={`font-bold text-navy-900 dark:text-zinc-100 group-hover:text-brand-indigo dark:group-hover:text-white transition-colors ${
              featured ? "text-xl" : "text-lg"
            }`}
          >
            {agent.name}
          </h3>
          <p className="text-xs font-semibold text-brand-dark dark:text-zinc-300 mt-0.5">
            {agent.role}
          </p>
        ...
        <div className="space-y-1.5 pt-3 border-t border-slate-100 dark:border-zinc-800">
          <div className="text-[10px] font-bold uppercase tracking-wider text-slate-500 dark:text-zinc-400 flex items-center gap-1">
          ...
          <div className="flex flex-wrap gap-1">
            {agent.deterministicAuditChecks.map((chk, i) => (
              <span
                key={i}
                className="text-[10px] font-mono bg-slate-50 dark:bg-zinc-900 text-slate-700 dark:text-zinc-300 border border-slate-200 dark:border-zinc-750 px-2 py-0.5 rounded"
              >
                {chk}
              </span>
            ))}
      ...
      <div className="pt-2">
        <button
          ...
          className="w-full inline-flex items-center justify-center gap-2 py-2 px-3 rounded-xl bg-slate-50 dark:bg-zinc-800 hover:bg-brand-light dark:hover:bg-zinc-750 text-slate-700 dark:text-zinc-200 hover:text-brand-dark dark:hover:text-white text-xs font-bold transition-all duration-150 border border-slate-200 dark:border-zinc-700 hover:border-brand-soft/60 dark:hover:border-zinc-600 active:scale-[0.98] cursor-pointer"
        >
