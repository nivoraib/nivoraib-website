...
export default function DashboardClient() {
  ...
  const [validationState, setValidationState] = useState<"generated" | "validating" | "validated" | "approved">(
    "validated"
  );
      ...
      <div className="flex-1 flex flex-col lg:flex-row max-w-[1600px] w-full mx-auto p-3 sm:p-6 gap-4 sm:gap-6">
        ...
        <aside className="w-full lg:w-64 bg-white dark:bg-zinc-850 rounded-2xl border border-slate-200 dark:border-zinc-700 p-4 shadow-sm flex flex-col justify-between shrink-0 space-y-6">
          <div className="space-y-4">
            <div className="flex items-center gap-2.5 px-2 pb-3 border-b border-slate-100 dark:border-zinc-800">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-[10px] text-slate-500 dark:text-zinc-400 font-mono">DP Educator Preview</span>
              </div>
            </div>
            <div className="space-y-1">
              <span className="px-2 text-[10px] font-bold uppercase tracking-wider text-slate-500 dark:text-zinc-400 block mb-1">Academic Modules</span>
              {sidebarItems.map((item) => {
                ...
                return (
                  <button
                    key={item.id}
                    onClick={() => { setActiveWorkflow(item.id); triggerValidationRun(); }}
          ...
          <div className="p-3 bg-slate-50 dark:bg-zinc-800 rounded-xl border border-slate-200 dark:border-zinc-700 text-xs space-y-2">
            <div className="flex items-center gap-2">
              <div className="w-7 h-7 rounded-full bg-brand-light dark:bg-zinc-700 text-brand-dark dark:text-zinc-100 font-bold flex items-center justify-center text-xs">IB</div>
        ...
        <main className="flex-1 space-y-6 flex flex-col">
          ...
          <div className="bg-white dark:bg-zinc-850 rounded-2xl border border-slate-200 dark:border-zinc-700 p-6 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="space-y-1">
              <div className="flex items-center gap-2">
                <span className="text-xs font-bold uppercase tracking-wider text-brand-dark dark:text-zinc-200 bg-brand-light dark:bg-zinc-800 px-2 py-0.5 rounded-full">Interactive Preview</span>
                <span className="text-xs text-slate-500 dark:text-zinc-400 font-mono">Status: Connected to DP Graph</span>
              </div>
              <h2 className="text-2xl sm:text-3xl font-extrabold text-navy-900 dark:text-zinc-100 tracking-tight">Good morning, Teacher</h2>
              <p className="text-xs sm:text-sm text-slate-500 dark:text-zinc-400">What would you like to plan, create, or assess today?</p>
            </div>
            <div className="flex items-center gap-2">
              <button
          ...
          <div className="space-y-2">
            <span className="text-[11px] font-bold uppercase tracking-wider text-slate-500 dark:text-zinc-400 px-1">Sample Teacher Workflows (Click to run):</span>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2.5">
              {quickActions.map((action, idx) => (
                ...
                >
                  ...
                  <p className="text-[11px] text-slate-500 dark:text-zinc-400 truncate mt-1">{action.prompt}</p>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-zinc-850 rounded-2xl border border-slate-200 dark:border-zinc-700 p-5 shadow-sm space-y-4">
            <div className="flex items-center justify-between text-xs">
              <span className="font-bold text-navy-900 dark:text-zinc-100 flex items-center gap-1.5">
                <Sparkles className="w-4 h-4 text-brand-indigo dark:text-zinc-300" /> Prompt & Instruction Harness
              </span>
            ...
            <div className="space-y-1.5 pt-1">
              <span className="text-[11px] font-bold uppercase tracking-wider text-slate-500 dark:text-zinc-400 block">Attached Context Layers:</span>
              <div className="flex flex-wrap gap-2 text-xs">
                <span className="px-2.5 py-1 rounded-lg bg-blue-50 dark:bg-blue-950/30 text-blue-800 dark:text-blue-300 border border-blue-200 dark:border-zinc-700 flex items-center gap-1.5 font-medium">
                  <BookOpen className="w-3 h-3 text-blue-600 dark:text-blue-400" /> IB DP Subject Guide (2025/2026)
                </span>
                <span className="px-2.5 py-1 rounded-lg bg-indigo-50 dark:bg-indigo-950/30 text-indigo-800 dark:text-indigo-300 border border-indigo-200 dark:border-zinc-700 flex items-center gap-1.5 font-medium">
                  <FolderSearch className="w-3 h-3 text-indigo-600 dark:text-indigo-400" /> Teacher Course Manual (Term 1)
                </span>
                <span className="px-2.5 py-1 rounded-lg bg-slate-100 dark:bg-zinc-800 text-slate-800 dark:text-zinc-200 border border-slate-200 dark:border-zinc-700 flex items-center gap-1.5 font-medium">
                  <Layers className="w-3 h-3 text-slate-600 dark:text-zinc-400" /> School Timetable: Period 3 (90 Min)
                </span>
                <span className="px-2.5 py-1 rounded-lg bg-amber-50 dark:bg-amber-950/30 text-amber-800 dark:text-amber-300 border border-amber-200 dark:border-zinc-700 flex items-center gap-1.5 font-medium">
                  <Clock className="w-3 h-3 text-amber-600 dark:text-amber-400" /> Previous Unit Assessment Archive
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-zinc-850 rounded-2xl border border-slate-200 dark:border-zinc-700 p-5 shadow-sm space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-100 dark:border-zinc-800 pb-3">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-bold text-navy-900 dark:text-zinc-100">Stage 1 Verification Engine</span>
              </div>
            ...
            <div className="grid grid-cols-4 gap-2 text-center text-xs font-semibold">
              <div className={`p-2.5 rounded-xl border transition-all ${validationState === "generated" ? "bg-brand-indigo dark:bg-zinc-600 text-white border-brand-indigo dark:border-zinc-500 shadow-sm" : "bg-slate-50 dark:bg-zinc-800 text-slate-600 dark:text-zinc-300 border-slate-200 dark:border-zinc-700"}`}>1. GENERATED</div>
              <div className={`p-2.5 rounded-xl border transition-all ${validationState === "validating" ? "bg-amber-500 text-white border-amber-500 animate-pulse shadow-sm" : "bg-slate-50 dark:bg-zinc-800 text-slate-600 dark:text-zinc-300 border-slate-200 dark:border-zinc-700"}`}>2. VALIDATING...</div>
              <div className={`p-2.5 rounded-xl border transition-all ${validationState === "validated" ? "bg-emerald-600 text-white border-emerald-600 shadow-sm" : "bg-slate-50 dark:bg-zinc-800 text-slate-600 dark:text-zinc-300 border-slate-200 dark:border-zinc-700"}`}>3. VALIDATED ✓</div>
              <div className={`p-2.5 rounded-xl border transition-all ${validationState === "approved" ? "bg-brand-dark dark:bg-white text-white dark:text-zinc-950 border-brand-dark dark:border-zinc-200 shadow-sm" : "bg-slate-50 dark:bg-zinc-850 text-slate-600 dark:text-zinc-300 border-slate-200 dark:border-zinc-700"}`}>4. APPROVED</div>
            </div>
          ...
          <div className="bg-white dark:bg-zinc-850 rounded-2xl border border-slate-200 dark:border-zinc-700 p-6 shadow-sm space-y-5">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 dark:border-zinc-800 pb-4">
              <div>
                <span className="text-[10px] font-bold uppercase tracking-wider text-brand-dark dark:text-zinc-200 bg-brand-light dark:bg-zinc-800 px-2 py-0.5 rounded">Validated Lesson Plan Draft</span>
                <h3 className="text-lg sm:text-xl font-extrabold text-navy-900 dark:text-zinc-100 mt-1">Topic B.1.2: Cellular Respiration, Glycolysis &amp; ATP Synthesis</h3>
              </div>
              <div className="flex items-center gap-2">
                <button
            ...
            <div className="space-y-4 text-xs sm:text-sm text-slate-700 dark:text-zinc-300">
              <div className="p-3.5 rounded-xl bg-slate-50 dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 space-y-1">
                ...
                <p className="text-xs text-slate-600 dark:text-zinc-300 leading-relaxed">
                  Screen interactive 3D model of mitochondria. Socratic prompt: “Explain why active muscle cells contain up to 2,000 mitochondria per cell while skin cells contain far fewer.” (Command Term: Explain).
                </p>
              </div>
              <div className="p-3.5 rounded-xl bg-slate-50 dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 space-y-1">
                <div className="flex items-center justify-between text-xs font-bold text-navy-900 dark:text-zinc-100">
                ...
                <p className="text-xs text-slate-600 dark:text-zinc-300 leading-relaxed">
                  Students split into paired lab stations tracking CO2 respirometer levels across glucose, maltose, and lactose broths.<br />
                  <strong className="text-slate-800 dark:text-zinc-200">Scaffolding (EAL):</strong> Visual chemical equation cards provided with dual-language glossary terms.<br />
            ...
            <div className="pt-2 flex flex-wrap items-center justify-between gap-3 text-xs text-slate-500 dark:text-zinc-400 border-t border-slate-100 dark:border-zinc-800">
              ...
              <span className="font-mono text-[11px]">Audit Pass Hash: #482f-verified</span>
            </div>
          </div>
        </main>
      </div>
